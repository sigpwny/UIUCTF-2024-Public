...
I wanted to learn crypto, so I wrote a crypto/pwn challenge that execs aes code encrypted with a key unknown to the players.

Provides a decryption oracle. Users must craft a ciphertext that decrypts
to valid x86 assembly to set rax to a specific value. For a more
difficult/interesting challenge (maybe a tier 2), force shellcode instead.

All you need to do is `cargo build`.

openssl enc -aes-128-cbc -in bin -out encrypted.bin -K 42694269426942694269426942694269 -iv 00000000000000000000000000000000
